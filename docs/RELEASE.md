...
5. Create the package with `npm pack`, calculate its SHA-256 checksum, and
   generate a CycloneDX or SPDX SBOM from the clean checkout.
6. Test the tarball on a clean supported Linux environment before signing the
...
outside the source tree, runs the known-positive workflow, reads durable status,
and renders a report. Its temporary package, state, and artifacts are deleted
after the check.
